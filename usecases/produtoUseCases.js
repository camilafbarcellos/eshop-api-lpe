...
const getProdutosDB = async () => {
    try {
        const { rows } = await pool.query(`SELECT p.codigo AS codigo, p.nome AS nome, 
        p.descricao AS descricao, p.quantidade_estoque AS quantidade_estoque, 
        p.ativo AS ativo, p.valor AS valor, to_char(p.data_cadastro,'YYYY-MM-DD') AS data_cadastro, 
